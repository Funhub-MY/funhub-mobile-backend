...
class MonthlyOfferReports extends Page implements HasTable
{
                ...
                ->query(function (Builder $query, array $data): Builder {
                    return $query->whereExists(function ($subQuery) use ($data) {
                        $subQuery->selectRaw(1)
                            ...
                            ->when(
                                $data['start_date'],
                                fn ($subQuery, $date) => $subQuery->where('merchant_offer_user.created_at', '>=', $date)
                            )
                            ->when(
                                $data['end_date'],
                                fn ($subQuery, $date) => $subQuery->where('merchant_offer_user.created_at', '<=', $date)
    ...
    protected function getTableQuery(): Builder
    {
        return MerchantOfferVoucher::query()
            ...
            ->leftJoin('merchant_offer_user', function ($join) {
                $join->on('merchant_offer_user.voucher_id', '=', 'merchant_offer_vouchers.id')
                    ->where('merchant_offer_user.user_id', '=', 'merchant_offer_vouchers.owned_by_id')
