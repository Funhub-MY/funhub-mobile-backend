...
namespace App\Filament\Resources\MerchantOfferCampaigns;
...
use Exception;
use App\Filament\Resources\MerchantOfferCampaigns\RelationManagers\VoucherCodesRelationManager;
use App\Filament\Resources\MerchantOfferCampaigns\Pages\ListMerchantOfferCampaigns;
use App\Filament\Resources\MerchantOfferCampaigns\Pages\CreateMerchantOfferCampaign;
use App\Filament\Resources\MerchantOfferCampaigns\Pages\EditMerchantOfferCampaign;
use Closure;
...
class MerchantOfferCampaignResource extends Resource
{
    ...
    public static function getEloquentQuery(): Builder
    {
        ...
        if (auth()->user()->hasRole('merchant')) {
            // Use merchant_id if available, fallback to user_id for backward compatibility
            $merchant = auth()->user()->merchant;
            if ($merchant && $merchant->id) {
                $query->where(function($q) use ($merchant) {
                    $q->where('merchant_id', $merchant->id)
                      ->orWhere('user_id', auth()->user()->id); // Fallback for legacy records
                });
            } else {
                    ...
                    ->schema([
                        Section::make()
                            ->schema([
                                ...
                                SpatieMediaLibraryFileUpload::make('horizontal_banner')
                                ...
                                // agreement quantity
                                TextInput::make('agreement_quantity')
                                    ->integer(true)
                                ...
                                Toggle::make('auto_move_vouchers')
                                ...
                                Textarea::make('description')
                                    ...
                                    ->columnSpan('full')
                                    ->required(),
                                Textarea::make('fine_print')
									->label('T&C')
                                    ->rows(5)
                                    ->cols(10)
                                    ->required()
                                    ->columnSpan('full'),
                                Textarea::make('redemption_policy')
                                    ->rows(5)
                                    ->cols(10)
                                    ->required()
                                    ->columnSpan('full'),
                        ...
                        Section::make()
                            ->schema([
                                ...
                                TextInput::make('unit_price')
                                    ...
                                    TextInput::make('point_fiat_price')
                                    ...
                                    TextInput::make('discounted_point_fiat_price')
                                ...
                                Fieldset::make('MYR Pricing')
                                    ...
                                    TextInput::make('point_fiat_price')
                                        ...
                                        TextInput::make('discounted_fiat_price')
                                        ...
                                        Group::make()
                                            ->schema([
                                                DateTimePicker::make('available_at')
                                                    ...
                                                    ->disabled(fn($livewire, Get $get) => $livewire instanceof EditRecord && $get('available_at') && Carbon::parse($get('available_at'))->isPast())
                                                    ->minDate(fn($get) =>  $get('available_at') ? Carbon::parse($get('available_at')) : now()->startOfDay()),
                                                DateTimePicker::make('available_until')
                                                    ->required()
                                                    ->columnSpan(1)
                                                    ->disabled(fn($livewire, Get $get) => $livewire instanceof EditRecord && $get('available_until') && Carbon::parse($get('available_at'))->isPast())
                                                    ->minDate(fn($get) => $get('available_at') ? Carbon::parse($get('available_at')) : now()->startOfDay()),
                                                ])
                                            ->columns(2),
                                        Group::make()
                                            ->schema([
                                                ...
                                                Placeholder::make('cannot_update_past')
                                                        ->visible(fn($livewire, Get $get) => $livewire instanceof EditRecord && $get('available_until') && Carbon::parse($get('available_at'))->isPast())
                                                        ->columnSpan(2)
                                                        ->disableLabel()
                    ...
                    ->schema([
                        Section::make('Merchant')
                            ->schema([
                                ...
                                Checkbox::make('select_all_stores')
                                    ...
                                    ->hidden(fn (Get $get) => $get('user_id') === null),
                                ...
                                Select::make('stores')
                                    ...
                                    ->preload()
                                    ->reactive()
                                    ->relationship('stores', 'name', function (Builder $query, Get $get) {
                            ...
                            Section::make('Global Vouchers')
                                ->schema([
                                    ...
                                    ->content(function (Get $get, $record) {
                                        ...
                                        $offers = MerchantOffer::where('merchant_offer_campaign_id', $record->id)
                                            ->withCount([
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('store.name')
                    ->default('-')
                    ->label('By Store'),
                TextColumn::make('merchant_status')
                ...
                ToggleColumn::make('auto_move_vouchers')
                ...
                TextColumn::make('vouchers_status')
                ...
                TextColumn::make('agreement_quantity')
                    ->label('Agreement Quantity'),

                TextColumn::make('upcoming_vouchers_count')
    ...
    {
        return [
            'index' => ListMerchantOfferCampaigns::route('/'),
            'create' => CreateMerchantOfferCampaign::route('/create'),
            'edit' => EditMerchantOfferCampaign::route('/{record}/edit'),
        ];
    }
}