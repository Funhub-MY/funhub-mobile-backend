...
namespace App\Services\Byteplus;
...
class SignatureV4
{
    ...
    public function signRequest(RequestInterface $request, $credentials)
    {
        $ldt = gmdate(self::ISO8601_BASIC);
        $sdt = substr($ldt, 0, 8);
        $parsed = $this->parseRequest($request);
        ...
        if (isset($credentials['st']) && $credentials['st'] != '') {
    ...
    protected function getPayload(RequestInterface $request)
    {
        ...
        if (!$request->getBody()->isSeekable()) {
            throw new Exception('sha256 hash error');
        }

        try {
            return hash('sha256', $request->getBody()->getContents());
        } catch (Exception $e) {
            throw new Exception('sha256 hash error', 0, $e);
        }
    }
    ...
    {
        if ($req['query']) {
            $req['uri'] = $req['uri']->withQuery(http_build_query($req['query']));
        }
